...
class RollerSkatingWorkoutSummaryController extends ControllerMVC {
  ...
  int index = 0;
  RollerSkatingObj selected = RollerSkatingObj(
      longitude: 0.0, latitude: 0.0, speed: 0.0, whenSec: 0);
  ...
  Future<void> initMap(WorkOut workout) async {
    ...
    (workout.data as RollerSkating).snapShots.forEach((element) {
      latlng.add(LatLng(element.latitude, element.longitude));
    });
    var first = (workout.data as RollerSkating).snapShots.first;
    if (first != null) {
  ...
  void changePosition(WorkOut workout, int _index) {
    selected = (workout.data as RollerSkating).snapShots[index];
